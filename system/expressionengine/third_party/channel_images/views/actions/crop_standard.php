...
<table cellspacing="0" cellpadding="0" border="0" class="ChannelImagesTable CITable">
	<thead>
		<tr>
			<th><?=lang('ci:resize:startx')?></th>
			<th <?=$thstyle?>><?=lang('ci:resize:starty')?></th>
			<th <?=$thstyle?>><?=lang('ci:resize:width')?></th>
	...
	<tbody>
		<tr>
			<td><?=form_input($action_field_name.'[start_x]', $start_x, 'style="border:1px solid #ccc; width:80%;"')?></td>
			<td><?=form_input($action_field_name.'[start_y]', $start_y, 'style="border:1px solid #ccc; width:80%;"')?></td>
...
<small><?=lang('ci:crop:standard_exp')?></small>